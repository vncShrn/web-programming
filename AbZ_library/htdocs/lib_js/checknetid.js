$(document).ready(function() {
	$('#netid-button').click(function() {
		...
		if ($(sign_form).data('formstatus') !== 'submitting') {
			...
			//setup variables
			var form = $(sign_form), responseMsg = $('#netid-response'), netid = $("#net_id").val();
			...
			$.ajax({
				...
				error : function() {
					//show reponse message
					responseMsg.fadeOut(200, function() {
						$(this).removeClass('response-waiting').addClass("error").text("Error calling POST php").fadeIn(200, function() {
							//set timeout to hide response message
							setTimeout(function() {
								responseMsg.fadeOut(200, function() {
									$(this).removeClass(klass);
